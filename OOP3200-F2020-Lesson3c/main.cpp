...
int main()
{
	...
	try
	{
		...
		//Opens file
		std::ifstream inClientFile{ "MockDataForTesting.txt", std::ios::out };

		// If file doesn't open, display error message
		if (!inClientFile)
		{
			std::cerr << "MockDataForTesting.txt could not be opened for input. Please check the file exists\n";
			std::cout << "---------------------------------\n";
			exit(EXIT_FAILURE);
		}

		// If map is empty
		// if(inClientFile empty)

			/*std::cerr << "The map is empty. Check that the file contains data in the correct format";
			std::cout << "---------------------------------\n";
			exit(EXIT_FAILURE);*/
		...
		std::cout << "The map contains a total of " << count << " points for a total distance of [MISSING DISTANCE]" << std::endl;
		...
		auto distance = Vector2D<int>::Distance(Vector2D<int>(0, 0), Vector2D<int>(10, 20));
